...
argparser.add_argument("-2", "--prune", action="store_true", help="prune model and retrain, iteratively")
argparser.add_argument("-t", "--train_iterations", default="20000", help="pre-train dense model with t iterations")
argparser.add_argument("-m", "--checkpoint", default="./models/model_ckpt_dense", help="Target checkpoint model file for 2nd and 3rd round")
...
if(args.train or args.prune) == False:
...
def transfer_to_sparse(weight):
    ...
    values = weight_array[weight_array != 0]
    indices = np.transpose(np.nonzero(weight_array))
    shape = list(weight_array.shape)
    return [indices, values, shape]

mnist = input_data.read_data_sets('./data/', one_hot=True)
file_object = open("logrpt.txt", "w")
sess = tf.Session()
...
if(args.train == True):
    ...
    for i in range(int(args.train_iterations)):
        ...
        if i % 1000 == 0:
            ...
            tm = time.clock()
            print("[Info] Step = %d, training accuracy = %g, time = %g ms" % (i, train_acc, tm*1000))
    ...
    print("Dense model:test accuracy = %g" % test_acc + ", inference time = " + str((end - start)*1000) + "ms")
    file_object.write("[Info]---------------------------Pre-training Result List----------------------[Report]\n")
...
if(args.prune == True):
    ...
    file_object.write("[Info]-------------------------Pruning and Retraining Result List--------------------[Report]\n")
    ...
    for i in range(int(args.iteration)):
        ...
        for grad, var in grads:
            if (var.name == "w_fc1:0"):
                ...
                print("sdchen22222 %s \n %s \n %s"%(idx_in1,grad,var))
                print("2222222222sdchen checking\n", grads[count][0])
            if (var.name == "w_fc2:0"):
